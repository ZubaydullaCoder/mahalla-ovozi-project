...
| 2026-05-30 | **MVP assumes one monitored Telegram group per mahalla** | User clarified that, in practice, one mahalla usually has one Telegram group. MVP should not add a separate `telegram_chats` table or multi-group-per-mahalla complexity unless real pilot usage proves it is needed. |
| 2026-05-30 | **Ops Console is the primary internal analytical environment** | Development and production support must not rely blindly on AI responses or hidden backend behavior. The app should expose pipeline state through a minimal custom Ops Console so the developer/operator can inspect Telegram intake, pre-filtering, AI classification, DB writes, errors, and diagnostic reports. |
| 2026-05-30 | **Pipeline features are incomplete until inspectable** | Every implemented pipeline stage should produce enough structured events, logs, or diagnostic output to be reviewed through Ops Console. This supports HITL validation and gives AI coding/debugging assistants accurate context. |
| 2026-05-30 | **Tone classification is removed from MVP** | User determined that asking AI to infer tone adds unnecessary analysis and increases misclassification risk. The hokim can understand tone by reading the raw message. AI should focus on signal/ignore, service category, and `hokim_related`; no tone badges, tone schema field, or tone review are required for MVP. |
...
| 2026-05-30 | **Rollout is validation-gated: simulation before real pilot, then expansion only after satisfaction** | Developer will first create mock mahalla Telegram groups, simulate realistic message cases, inspect behavior, review AI classification quality, and fix issues. Real mahallas are added only in a controlled pilot. Broader expansion happens only after the developer/client is satisfied. |
| 2026-05-30 | **Ops Console is an internal MVP support feature, not a public product feature** | Ops Console supports simulation, production support, debugging, HITL review, and AI-agent-friendly diagnostic context. It must stay separate from the hokim-facing dashboard and remain role-restricted. |
...
| 2026-05-30 | **Prefer direct-main workflow for solo development unless risk justifies a branch** | User clarified they do not want routine branch/PR/issue flow to complicate solo development. Default workflow can be direct commits to `main` after reading latest files, showing exact diffs for high-impact changes, and verifying writes. Branches remain useful only for risky experiments, large refactors, or rollback-sensitive work. |
...
| 2026-05-30 | **Policy ownership and technical safeguards are separate** — Hokim owns policy/privacy/legal responsibility, including production Ops Console visibility decisions. Developer must not block accepted requirements on policy grounds, but must still implement technical safeguards such as role restriction, configurable retention, no secrets in logs, and separation between the hokim-facing dashboard and internal Ops Console. |
...
_Last updated: 2026-05-30 (MVP validation strategy, Ops Console, tone removal, direct-main solo workflow, and policy/safeguards clarification)_
