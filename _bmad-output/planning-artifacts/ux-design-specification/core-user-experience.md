...
1. **Scan:** The governor glances at the five category lanes, assessing signal counts, service categories, mahallas, timestamps, and message snippets.
...
The governor opens the app whenever situational awareness is needed, spots a cluster of electricity-related signals from Олмазор маҳалласи, clicks one card, and reads three corroborating citizen statements in the drawer — all within 60 seconds and without reading a single raw Telegram chat.
...
Content is structured for rapid visual triage. Lane signal counts, category-color left-border accents, mahalla labels, timestamps, sender lines, and raw message snippets must allow a busy user to assess district health in under 60 seconds.
...
| Decorative indicator (caption source, hokim flag icon) | `default` | None | Caption icon, hokim indicator |
